...
public class MD5Util
{
  ...
  public static Boolean timespan(String timestamp) throws ParseException {
      Date date = new Date();
      Date timeStampDate;
     SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
     timeStampDate= sdf.parse(timestamp);
     Calendar calendar  =   Calendar.getInstance();
    calendar.setTime(timeStampDate); //需要将date数据转移到Calender对象中操作
    calendar.add(Calendar.HOUR_OF_DAY, 1);//把日期往后增加n天.正数往后推,负数往前移动
    timeStampDate=calendar.getTime();   //这个时间就是日期往后推一天的结果
    return timeStampDate.after(date);
  }
  
  public static String toMD5mHex(String data){
